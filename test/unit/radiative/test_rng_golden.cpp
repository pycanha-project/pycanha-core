...
[[nodiscard]] std::uint32_t rng_init(std::uint32_t face_slot,
                                     std::uint32_t ray_index,
                                     std::uint32_t batch_seed) {
    return pcg_hash(face_slot ^ pcg_hash(ray_index ^ pcg_hash(batch_seed)));
}
...
void check_golden(const Golden& golden) {
    std::uint32_t state =
        rng_init(golden.face_slot, golden.ray_index, golden.batch_seed);
