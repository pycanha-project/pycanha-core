...
TEST_CASE("Cube exposes a deterministic face atlas", "[gmm][primitive][cube]") {
    ...
    SECTION("uv conversion respects orientation") {
        const Cube rotated_cube({0.0, 0.0, 0.0}, {2.0, 4.0, 6.0}, quarter_turn);

        tests::require_round_trip(
            rotated_cube, std::array<Point3D, 3>{Point3D(0.0, 1.0, 0.0),
                                                 Point3D(-2.0, 0.0, 1.0),
                                                 Point3D(1.0, 0.5, 3.0)});
    }
    ...
    SECTION("normal rotates with orientation") {
        const Cube rotated_cube({0.0, 0.0, 0.0}, {2.0, 4.0, 6.0}, quarter_turn);
        const Point2D uv = rotated_cube.to_uv(Point3D(0.0, 1.0, 0.0));
        tests::require_parallel(rotated_cube.normal_at_uv(uv),
