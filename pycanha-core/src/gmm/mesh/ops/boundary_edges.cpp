...
void collect_edges(const TriMesh& mesh,
    ...
    for (Eigen::Index tri_idx = 0; tri_idx < mesh.triangles.rows(); ++tri_idx) {
        ...
        const std::array<Edge, 3> edges = {{{triangle[0], triangle[1]},
...
[[nodiscard]] std::vector<Edge> collect_boundary_edges(
    const std::vector<Edge>& directed_edges,
    const std::unordered_map<Edge, int, EdgeHash>& edge_use_counts) {
    std::vector<Edge> boundaries;
    boundaries.reserve(directed_edges.size());
    std::copy_if(
        directed_edges.begin(), directed_edges.end(),
        std::back_inserter(boundaries), [&edge_use_counts](const Edge& edge) {
            return edge_use_counts.at(normalized_edge(edge[0], edge[1])) == 1;
        });
    return boundaries;
}

[[nodiscard]] bool extend_loop(const std::vector<Edge>& boundaries,
    ...
    for (std::size_t edge_idx = 0; edge_idx < boundaries.size(); ++edge_idx) {
        if ((used[edge_idx] != 0U) || (boundaries[edge_idx][0] != loop_end)) {
            continue;
        }
...
[[nodiscard]] std::vector<Eigen::Index> trace_loop(
    const std::vector<Edge>& boundaries, std::vector<unsigned char>& used,
    std::size_t start_idx) {
    std::vector<Eigen::Index> loop{boundaries[start_idx][0],
                                   boundaries[start_idx][1]};
    used[start_idx] = 1U;

    while (extend_loop(boundaries, used, loop)) {
    }
...
std::vector<std::vector<Eigen::Index>> boundary_edge_loops(
    ...
    collect_edges(mesh, edge_use_counts, directed_edges);
    const std::vector<Edge> boundaries =
        collect_boundary_edges(directed_edges, edge_use_counts);
    ...
    for (std::size_t start_idx = 0; start_idx < boundaries.size();
        ...
        std::vector<Eigen::Index> loop =
            trace_loop(boundaries, used, start_idx);
